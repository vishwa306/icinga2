...
void ApiListener::HandleConfigUpdate(const MessageOrigin::Ptr& origin, const Dictionary::Ptr& params)
{
	 ...
	 */
	auto lock (Shared<std::unique_lock<std::mutex>>::Make(m_ConfigSyncStageLock));
 ...
 */
void ApiListener::AsyncTryActivateZonesStage(const std::vector<String>& relativePaths, const Shared<std::unique_lock<std::mutex>>::Ptr& lock)
{
